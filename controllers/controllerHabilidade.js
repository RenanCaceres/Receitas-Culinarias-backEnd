...
module.exports = {
    async getRelatorio(req, res) {
        try {
            ...
            const totalAlunos = await db.Usuario.count({
                where: { tipo: 2 }
            });
            ...
            for (const habilidade of habilidades) {
                ...
                const somaNiveis = registros.reduce((total, item) => {
                    return total + Number(item.nivel || 0);
                }, 0);

                const proporcao = totalAlunos > 0
                    ? Math.round((alunosComHabilidade / totalAlunos) * 100)
                    : 0;

                const nivelMedio = alunosComHabilidade > 0
                    ? (somaNiveis / alunosComHabilidade).toFixed(1)
                    : '0.0';
    ...
    async getUpdate(req, res) {
        try {
            ...
            if (!habilidade) {
                return res.redirect('/habilidadeList');
            }
